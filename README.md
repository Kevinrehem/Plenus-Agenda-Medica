...
**Sprint Goal:** Desenvolver as funcionalidades essenciais para a gestão e visualização de agendamentos, permitindo que administradores criem e gerenciem horários e que clientes possam solicitar, visualizar e cancelar suas consultas. 
...
-   Implementar o formulário para criação/edição de um novo agendamento (cliente, data, hora, serviço). 
...
-   Implementar a lógica para enviar a solicitação para aprovação do Administrador. 
...
-   Após o envio, o agendamento deve ser criado com o status inicial (ex: "Aguardando Confirmação"). 

#### História de Usuário 3: Visualização e Cancelamento de Agendamentos pelo Cliente 

**Título:** Como **Cliente**, eu quero visualizar e cancelar meus próprios agendamentos para ter controle sobre minhas consultas. 
...
-   Agendamentos cancelados devem ter seu status atualizado. 
...
**Tarefas:** 
-   Definir os diferentes status possíveis (ex: Confirmado, Cancelado, Realizado, Aguardando Pagamento, etc.). 
-   Implementar a lógica para atualizar o status de um agendamento conforme as ações são realizadas (criação, cancelamento, etc.). 
...
-   O status deve mudar corretamente quando um agendamento é criado, confirmado ou cancelado. 